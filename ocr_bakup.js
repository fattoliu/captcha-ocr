...
app.post('/ocr', async (req, res) => {
    ...
    try {
        ...
        // 如果当前图片和上一次的图片不一致，删除上一次处理过的图片
        if (currentImageHash !== lastImageHash) {
            const processedImages = fs.readdirSync(__dirname).filter(file => file.startsWith('processed_image_'));
            processedImages.forEach(file => {
                fs.unlinkSync(path.join(__dirname, file));
                console.log(`Deleted processed image: ${file}`);
            });
            lastImageHash = currentImageHash;
        }

        // 保存原始图像
        const originalImagePath = path.join(__dirname, 'original_image.png');
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            // 保存处理后的图像
            const processedImagePath = path.join(__dirname, `processed_image_${attempt}.png`);
